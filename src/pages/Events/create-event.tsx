	...
	return (
		...
		>
			<Stack direction='row' justifyContent='space-between' alignItems='center' spacing={2}>
				<DataLabelDisplay data={props.participantName} label={props.participantState} />

				<DataLabelDisplay data={props.participantEmail} label='E-mail do participante' />
			...
			content: (
				...
				>
					<FormControl>
						...
						<Input placeholder='Nome do Evento' size='sm' variant='outlined' />
					</FormControl>

					<FormControl>
						<FormLabel>Descrição</FormLabel>
						<Textarea placeholder='Descrição do evento' minRows={6} variant='outlined' />
					</FormControl>

					<Stack direction='row' justifyContent='flex-start' alignItems='center' spacing={2}>
						<FormControl>
							<FormLabel>Data do evento</FormLabel>
							<Input placeholder='Exemplo' size='sm' variant='outlined' />
						</FormControl>
						<FormControl>
				...
				>
					<Stack direction='row' justifyContent='flex-start' alignItems='center' spacing={2}>
						<FormControl>
							...
							<Input placeholder='Nome completo' size='sm' variant='outlined' />
						</FormControl>

						<FormControl>
							<FormLabel>E-mail para participar</FormLabel>
							<Input placeholder='E-mail' size='sm' variant='outlined' />
						</FormControl>
					...
					<Box component='ul' sx={{ m: 0, p: 0 }}>
						{participantsMock.map((participant) => (
							<ParticipantListItem key={participant.participantId} {...participant} />
						))}
					</Box>
				</Box>
			),
	...
	return (
		<PageWrapper breadcrumbLabel='Criar Evento'>
			...
			<Stepper sx={{ width: '100%', paddingY: 3 }}>
				{steps.map((step, index) => (
					<Step
						...
						sx={{ color: 'black' }}
						indicator={
							<StepIndicator variant={activeStep <= index ? 'soft' : 'solid'} color={activeStep < index ? 'primary' : 'success'}>
			...
			<Stack direction='row' spacing={2} sx={{ mt: 2 }}>
				...
				<Button variant='solid' color='primary' onClick={handleNext} disabled={activeStep === steps.length - 1}>
					Próximo passo
				</Button>
			</Stack>
		</PageWrapper>
