...
import { Avatar, Box, Button, Grid,Sheet, IconButton, Typography , useTheme} from "@mui/joy";
		...
		<PageWrapper breadcrumbLabel="Visualização de Certificado">
			...
			<Box component="ul" sx={{ m: 0, p: 0 }}>
        		{certificatesMock.map((certificate) => (
          			<CertificateListItem key={certificate.certificateId} {...certificate} />
        		))}
      		</Box>
		</PageWrapper>
	);
		...
		<Grid>
			...
			>
				...
				<Box sx={{ position: 'absolute', top: 20, left: 50 }}>
					...
					<img src={logo} alt="Logo da Universidade" style={{ height: '30px' }} />
				</Box>
				...
				>
					<Typography component="div" sx={{ mt: '80px' , fontSize: '2rem'}} >
					Certificado
					</Typography>
					...
					<Box sx={{ p: 3 }}>
						...
						Certificamos que <strong>{props.participantName}</strong> participou do evento <strong>"{props.eventName}"</strong>, realizado no dia <strong>{props.eventDate}</strong>, com carga horária de <strong>{props.creditHours} horas</strong>.
						</Typography>
						<Typography sx={{ textAlign: 'right', mt: 2 , fontWeight: 'bold'}}>
							{props.location}, {props.issueDate}
						</Typography>
						<Box sx={{ textAlign: 'center', borderTop: 1, borderColor: 'neutral.outlinedBorder', pt: 2, mt: 2 }}>
			...
			>
				...
				<Button variant="solid" color="neutral" onClick={handleDownload}>Download</Button>
				<Button variant="solid" onClick={handleShare}>Compartilhar</Button>
				</Box>
			</Grid>
		</Grid>
	  );
